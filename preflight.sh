...
# 1) .asc должен соответствовать .cir: перегенерация + diff и сборка нетлиста из геометрии (verify_asc.py)
...
python3 verify_asc.py >/dev/null || { echo "FAIL: verify_asc.py — нетлист из .asc не совпал с .cir"; rc=1; }
